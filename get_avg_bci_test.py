THREE_BRIDGES = [[1, 'Highway 24 Underpass at Highway 403', '403', 43.167233,
                 ...
                 [4, 'STOKES RIVER BRIDGE', '6', 45.036739, -81.33579, '1958',
                  '2013', '', 1, [16.0], 18.4, '08/28/2013',
                  []],                 
                ]


def get_average_bci(bridge_data, bridge_id):
    ...
    for data_set in range(len(bridge_data)):
        for bci_val in bridge_data[data_set][12]:
            if bridge_id == bridge_data[data_set][0]:
                bci_sum = sum(bridge_data[data_set][12])
                bci_amount = len(bridge_data[data_set][12])
                return float(bci_sum/bci_amount)
            #elif bridge_data[data_set][12] == []: ---> is this code necessary
                #return 0.0
    return 0.0
        
print(get_average_bci(THREE_BRIDGES, 4))